...
### U-10. card-fraud-realism-v2: the merchant, IP and label dispositions (card-fraud-2026-07k)

Model choices adopted 2026-07-26 (roadmap docs/card_fraud_v2_roadmap.md;
gates 1, 4, 5 of docs/card_fraud_online_gnn.md; owner directive: the
deliverable is realistic data a GNN can be trained on honestly).
Covers the two generation shortcuts closed in this arc and the exporter
label dispositions. Rounds 2 (point-in-time contract) and 3 (prevalence
suite) are declared here and land next; both are zero-golden.

| PL value | Class | Suggested source |
|---|---|---|
| FRAUD CARD-RAIL DESTINATIONS: the unauthorized card rail and the gift-card scam select from the SAME merchant acceptance catalogue as legitimate spend, modality-conditioned — card-present draws over physical outlets weighted `weight x exp(-miles / cardPresentDecayScaleMiles(homeArea))` (the SAME kernel the legitimate session uses, off the victim's home area), card-not-present draws over the `Footprint::online` population by popularity, gift-card scam always card-present. Degrades to the blueprint biller pool only when no catalogue, no eligible merchant or no home area is available. Own lane `{"fraud","unauth","merchant",seq}` so ring scheduling is unperturbed | MEASUREMENT (defect repair) + CHOICE (mechanism) | the audit's own SQL finding (748/748 fraud rows on merchants with zero legitimate card rows); geo-causal-v1 decay curve |
| CARD-NOT-PRESENT SHARE = 0.70 for the unauthorized rail: a DECLARED CHOICE, not an anchored value. The anchored claim is the DIRECTION — card-not-present dominates modern card fraud — and the arc's gates measure separability, not this number. A calibrated CNP share against Nilson/FTC series is REGISTERED | CHOICE (declared, direction-anchored) | Nilson Report / FTC Consumer Sentinel card-fraud channel mix (registered) |
| ANTI-REPLACEMENT CONDITION (why not a flat national draw): legitimate card-present spend is distance-decayed, so uniformly-placed fraud would have traded the merchant-identity shortcut for a DISTANCE-FROM-HOME shortcut. Routing fraud through the same kernel is what makes the closure real. The standing law: closing one shortcut must not open another — ship the baseline gate that would catch the replacement | INVARIANT (arc law) | tests/test_card_baselines.cpp |
| ATTACKER IP SHORTCUT CLOSED: the injector wrote `Ipv4::pack(198, 51, 100, ...)` — TEST-NET-2 — for every unauthorized event, while legitimate IPs come from `randomIpv4` (first octet 11-222, full 32-bit spread). Collision probability ~1 in 14,000,000: a /24 lookup labelled unauthorized fraud essentially perfectly. Now `network::randomIpv4(rng)`. Deliberate ring INFRASTRUCTURE SHARING is untouched — that is real behavioral signal, not a shortcut, and any future IP baseline must not treat it as one | MEASUREMENT (defect repair) | the online-GNN audit's TEST-NET finding, re-verified in the constructor form |
| PROCESS LAW (recorded because it nearly cost the fix): an earlier round declared the TEST-NET claim stale after a repo-wide grep for the DOTTED-QUAD strings found only the audit doc. The defect was written in integer-octet form. GREP THE CONSTRUCTOR, NOT THE RENDERED LITERAL, before calling an audit claim stale | INVARIANT (review law) | this arc |
| ENTITY LABELS WITHHELD FROM THE FEATURE GRAPH: `Card.is_fraud`, `Party.is_fraud`, `Device.is_blocked`, `IP.is_blocked` are FULL-WINDOW verdicts ("this card ever carried a flag-1 row"), which answer the training question before a model sees a transaction. All four columns are RETAINED — TF_GNN_v3 loading jobs map POSITIONALLY, owner ruling — and written as 0. The ONE supervised target inside the graph is `Payment_Transaction.is_fraud`, observable at its own row's timestamp | CHOICE (owner ruling: retain-and-zero, not drop) + INVARIANT (no full-window label in the feature graph) | docs/card_fraud_online_gnn.md gate 1 |
| THE GROUND-TRUTH OVERLAY: the withheld verdicts move to a 35th table, `card_fraud."cf_Ground_Truth_Label"` — `(entity_type, entity_id, label)`, POSITIVES ONLY (absence = 0), joinable 1:1 to the vertex tables. Vocabulary: entity_type in {card, party, device, ip}, label in {ever_fraud, fraud_actor, flagged, blacklisted}. No TF_GNN_v3 job loads it and no edge points at it; it exists for EVALUATION. Party labelling keeps the card-fraud-2026-07 definition — fraud ACTORS (ring member, solo fraudster, mule), never victims | CHOICE (quarantine mechanism) | this repository |
| WITHHOLDING IS NOT DELETION (the gate that says so): tests/test_pipeline_e2e.cpp asserts every cell of the four columns renders 0 AND that whenever the window produced flagged card rows the overlay carries the cards they touched; docs/card_fraud_postgres_acceptance.sql enforces the same on a live corpus plus overlay-to-vertex join integrity and the label vocabulary. Table count 34 -> 35 in both | INVARIANT (gate) | tests/test_pipeline_e2e.cpp |
| GOLDEN IMPACT, DECLARED PER ROUND: the merchant selection and the IP repair are MODEL-MOVING (all four goldens re-pin); the label round is EXPORTER-ONLY (the three TABLE goldens re-pin and the STREAM golden must NOT move — if golden_run.b2sum moves on that round it is a defect, not a re-pin) | INVARIANT (byte-identity harness) | tests/CMakeLists.txt |
| STALE PROHIBITION LIFTED: the audit's `Party.created_at` prohibition predates H3. The card_fraud exporter writes `membership.joinTs(p)` through H3's single membership construction path — when the customer relationship began, not a window-wide constant. Formally lifted with round 2's point-in-time contract | MEASUREMENT (code fact) | src/exporter/card_fraud/export.cpp |
| DECLARED REMAINING (rounds 2-3, both zero-golden): the POINT-IN-TIME CONTRACT (the feature-safe column list plus a test that appending future events cannot change an earlier score-time feature) and the PREVALENCE SUITE (per-year fraud rate on the Payment view plus channel/typology/amount/episode-size bands — H4 made volume era-varying, so one aggregate rate is uninformative). `use_chip` is now MODEL-BACKED on the fraud rail (b-2 draws real modality) but still a content hash for legitimate rows — partial, tracked; `error` stays prohibited until authorization attempts are modeled | CHOICE (declared scope) | docs/card_fraud_v2_roadmap.md |
