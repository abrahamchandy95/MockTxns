...
public:
  ...
  public:
    ...
    /* The day frame's CPI level multiplier (class P), computed once at
     * construction — pure derived data, no draws. Carried onto every
     * Event (ticket scaling) and Snapshot (screen scaling). */
    [[nodiscard]] double dayPriceScale() const noexcept;

    /* The frame's day index, so the loop can compare it to each
     * spender's retirement day (pure derived data, no draws). */
  ...
  private:
    ...
    /* Authority U-9: the day frame's REAL per-capita consumption level
     * (econ::realPceLevel), computed once at construction like
