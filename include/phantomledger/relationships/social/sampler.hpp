...
private:
  ...
  /* O(1) membership scratch for `drawUnique`'s dedup. An index is "present"
   * in the current call when `seenMark_[idx] == seenEpoch_`; bumping
   * `seenEpoch_` each call logically clears the set without touching the
   * buffer (except on the rare epoch wraparound, handled in `drawUnique`).
   * `mutable` because `drawUnique` is const but must mutate this scratch.
