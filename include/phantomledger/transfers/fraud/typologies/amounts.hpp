...
// card-compromise, account-takeover, gift-card-scam and impostor-push
// rails; the ring-run transaction-fraud step (txn_fraud_ring) is
...
// (cardFraudSpend, atoDrainAmount, scamWireAmount) take the event
// year's CPI level multiplier — the calibration-year median AND its
// clamps scale together, so the tail shape is era-invariant. The
// DENOMINATION samplers (cardTestCharge, giftCardScamAmount) stay
// FIXED-NOMINAL (owner-approved 2026-07-25): round probe amounts and
// rack denominations are physical artifacts like the $20 note — scaling
...
///
/// The victim's AGE-GRADED severity does NOT enter here, deliberately:
/// the rack caps a card at $500 whoever is buying, so on this rail the
/// age gradient rides the NUMBER of cards the victim is coached into
/// buying (the planner scales targetEvents; victimization-v3).
///
...
///
/// THREE MULTIPLIERS, all applied to the median AND both clamps so the
/// tail SHAPE is invariant and only its LEVEL moves:
///   priceScale  the event year's CPI level (H1 step 2b, class F).
...
/// declared CHOICEs; authority row victimization-v3):
