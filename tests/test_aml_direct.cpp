//
// tests/test_aml_direct.cpp
//
// CSV retirement arc, step 3 acceptance (skips with 77 when no server
// is reachable; honors PL_TEST_PG). The aml exporter is fully migrated:
// the six transaction-streamed tables (transaction vertices, the four
// send/receive edge tables, chain labels — each open across the whole
// fold on its own connection) plus the ~46 finisher tables all flow
// through common::Table into schema aml with the csv_loader tree
// naming (aml_vertices_<stem> / aml_edges_<stem>). The raw ledger CSV
// (--show-transactions) stays file-only, same exclusion as standard.
//
// The gate runs the real binary (DEFAULT engine, live server), then
// loads the CSV tree the run just wrote into a scratch schema via the
// same csv_loader the old mirror pass used, and requires EVERY loaded
// table to match its direct twin exactly (row count + order-
// insensitive content hash). Generic over the loader's report — no
// hard-coded table list can go stale.
//
...
constexpr const char *kScratchSchema = "pl_aml_mirror_check";
...
int main() {
  ...
  const fs::path outDir = fs::temp_directory_path() / "pl_aml_direct";
  const fs::path logPath = fs::temp_directory_path() / "pl_aml_direct.log";
  ...
  const std::string cmd =
      ...
      "\" --usecase aml --population 1000 --days 60"
      " --seed 20260723 --show-transactions --out \"" +
      outDir.string() + "\" > \"" + logPath.string() + "\" 2>&1";

  std::printf("=== aml direct tables vs csv_loader (live PostgreSQL) ===\n"
              "  running binary ...\n");
  std::fflush(stdout);
  ...
  assert(reports.size() >= 40); // ~50 vertex/edge tables

  bool ok = true;
  std::size_t mismatches = 0;
  for (const auto &report : reports) {
    const auto directCount = count(*conn, "aml", report.table);
    const auto loadedCount = count(*conn, kScratchSchema, report.table);
    const auto directHash = contentHash(*conn, "aml", report.table);
  ...
  if (!ok) {
    std::fprintf(stderr, "AML DIRECT TABLES DIVERGE FROM FILES\n");
    return 1;
  }

  std::printf("AML DIRECT HOLDS: %zu tables cell-identical between the "
